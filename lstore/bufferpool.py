...
def frame_from_dict(self, dict, table_name: str, position: int, num_columns: int, from_disk: bool = True) -> Frame:
        # TODO
        frame = Frame(0, self.table_name, from_disk = True)
...
class BufferPool:
    ...
    def get_frame(self, table_name: str, page_range_index: int, num_columns: int):
        ...
        if table_name in self.frames:
            ...
            raise Exception("Frame does not exist in table")
            # VERIFY TODO Can't happen, exception? ignore idk
            #pass
    ...
    def request_record(self, t: Table, i: Index):
        # VERIFY TODO: used in query.py to request record of page_ranges, access page ranges using self.table
        print("returning page ranges")
        return self.table.page_ranges
