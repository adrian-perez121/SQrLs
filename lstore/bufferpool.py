...
class BufferPool:
    ...
    def has_capacity(self):
        count = sum(1 for table_frames in self.frames.values() for frame in table_frames if frame)
        return count < self.capacity  # "<" instead of "<="
    ...
    def get_frame(self, table_name: str, page_range_index: int, num_columns: int):
        ...
        if frame is None:
            ...
            # ✅ Fix: If still None, create a new Frame
            if frame is None:
                print(f"Creating new frame for {table_name} [{page_range_index}]")
