...
class BufferPool:
    ...
    def get_frame(self, table_name: str, page_range_index: int, num_columns: int):
        ...
        if table_name in self.frames:
            ...
            # TODO:2PL | LOCK (Mutex) - Acquire lock for page_range_index -> Only one worker can load from disk at once, if needed.
            if not frame:
                # TODO:Eviction evict
                # print("reading frame from disk")
                self.read_frame(table_name, page_range_index, num_columns)
                frame = frames[page_range_index]
            # TODO:2PL | RELEASE LOCK (Mutex) - Release lock for page_range_index -> Release so all other works may use the frame
