...
class Table:
    ...
    def add_new_page_range(self):
        if not self.bufferpool.get_frame(
            self.name, self.page_ranges_index, self.num_columns
        ).page_range.has_base_page_capacity():
        ...
        for pr in self.page_ranges:
            ...
            for base_page in pr.base_pages:
                for slot in range(base_page.num_records):
                    record = base_page.read_record_at(
                        slot, list(range(self.num_columns))
                    )
            ...
            for tail_page in pr.tail_pages:
                ...
                for slot in range(tail_page.num_records):
                    tail_records.append(
                        tail_page.read_record_at(slot, list(range(self.num_columns)))
                    )
                for tail_record in reversed(tail_records):
                    base_index = tail_record[RID_COLUMN]
                    if (
                        tail_record[TIMESTAMP_COLUMN]
                        > merged_records[base_index][TIMESTAMP_COLUMN]
                    ):
            ...
            for tail_page in pr.tail_pages:
                for slot in range(tail_page.num_records):
                    tail_record = tail_page.read_record_at(
                        slot, list(range(self.num_columns))
                    )
